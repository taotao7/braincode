...
export function Home() {
  ...
  const copyInstall = async () => {
    try {
      await navigator.clipboard.writeText("npm i -g @taotao7/braincode");
      ...
      <section className="section hero">
        <div className="container hero-split">
          <div>
            ...
            <div className="hero-cta" style={{ marginTop: 40, position: "relative" }}>
              <button 
                className={`btn btn-primary ${copyStatus === 'success' ? 'btn-success' : copyStatus === 'error' ? 'btn-error' : ''}`} 
                onClick={copyInstall}
                aria-label={t("btn_npm_install")}
              >
                {copyStatus === "success" ? t("btn_copied") : copyStatus === "error" ? t("btn_copy_failed") : t("btn_npm_install")}
              </button>
              <a href="#/docs" className="btn btn-secondary btn-arrow">{t("btn_docs")}</a>
              <div aria-live="polite" className="sr-only">
                {copyStatus === "success" && "Copied to clipboard."}
                {copyStatus === "error" && "Failed to copy. Please manually select and copy the text."}
              </div>
            </div>
