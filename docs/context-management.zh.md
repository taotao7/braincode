...
2. **支持文件组装** —— `readUserSupport` 收集 `~/.braincode/AGENTS.md`、`~/.braincode/mcp.json` 元数据、`~/.braincode/skills/*`；`readProjectSupport` 收集项目 `AGENTS.md`、`.mcp.json` 元数据、`.agents/skills/*` 内容。`formatProjectSupportPromptSection` 把 user/project 支持内容合并成 prompt 文本；`projectSupportContextRefs` 把它打包成 `ContextRef[]` 进 handoff packet。
