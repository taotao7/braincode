...
- **Transcript folding** in the TUI — tool and agent rows with `▸` / `▾` markers are toggled with `Ctrl+T`; mouse capture is off by default to preserve native terminal text selection and can be enabled for wheel scrolling with `BRAINCODE_TUI_MOUSE=true`.
