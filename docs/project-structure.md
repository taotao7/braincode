...
The interactive TUI is implemented with Ink and should expose Braincode product concepts such as mode, Brain Model routing, agent roles, tool approval, transcript folding, live token/elapsed status, and session state. Rendering should keep high-frequency data flows separated: input draft/cursor state, transcript/scroll state, live run status, toast/queue state, and BrainPet snapshots each have their own store and component subscription so typing and provider token streams do not force the whole frame to re-render. Ordinary input edits should patch the input box directly and reserve Ink redraws for layout changes. The running status line should tick independently of provider/tool events so elapsed time stays current during long quiet calls. Transcript folding is controlled with `Ctrl+T`, while mouse capture is off by default to preserve native terminal text selection; `BRAINCODE_TUI_MOUSE=true` enables wheel scrolling. The TUI should let users switch Braincode mode between `auto` and `radical` without leaving the TUI. It should not expose generic Pi model-switching controls; provider/model configuration belongs in `braincode config`.
