...
2. **Assemblage du support** — `readUserSupport` collecte `~/.braincode/AGENTS.md`, les métadonnées de `~/.braincode/mcp.json` et `~/.braincode/skills/*`; `readProjectSupport` collecte le `AGENTS.md` du projet, les métadonnées de `.mcp.json` et le contenu de `.agents/skills/*`. `formatProjectSupportPromptSection` formate le support user/projet combiné pour les prompts ; `projectSupportContextRefs` l'emballe en `ContextRef[]` pour les handoff packets.
