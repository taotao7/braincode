...
`radical` is the more aggressive mode. It still respects safety and tool permission boundaries, but it actively changes routing budgets and router behavior: routeBrain receives radical-specific decomposition instructions, the todo budget grows from 6 to 8, the effective worker budget is raised to at least 4 routed workers, support-worker concurrency is raised to at least 4 where dependencies allow, and each agent runs its own tool calls in parallel.

Use `radical` for users who prefer higher autonomy and faster end-to-end execution. The mode should add specialist workers earlier (`librarian`, `oracle`, `qa`, and domain experts when relevant) and keep dependencies sparse so independent support agents can run concurrently. Risky actions still go through the tool permission system and explicit approval policy where required.
...
Tool execution emits a separate, user-visible surface from normal assistant text. The TUI labels tool transcript rows by action class (`Web Search`, `Execute`, `Read`, `Write`, `MCP`, or generic `Tool`) and keeps tool start, streaming update, completion, failure, duration, arguments, and result summaries scannable. Risky tool categories can pause through the runtime's tool approval callback before Pi executes the call; the TUI presents those approval requests as checkable `Ask User` decisions and returns an approve/block result to the runtime. Tool approval honors `~/.braincode/tools.json`: tools enabled with `approvalPolicy: "allow"` in the Web UI are not prompted again. Non-interactive `braincode run` is read-only by default; `--allow-edits` exposes first-party local read/write tools and auto-approves file edits while blocking command execution, MCP tools, and unknown tools, and `--yes` exposes configured local/MCP tools with auto-approval.

Read-only evidence tools are available to `librarian`, `qa`, `security`, and `review` workers. These workers can list files, read files, search content/paths, inspect diffs, and inspect changed files, but they cannot edit files, apply patches, run shell commands, or run package scripts. The primary worker receives the configured local/MCP toolset according to the active permission mode.
