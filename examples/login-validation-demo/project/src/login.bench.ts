...
test("login trims email before authentication", () => {
  const result = login({ email: " person@example.com ", password: "secret" }, (email) => {
    return email === "person@example.com" ? "user-1" : undefined
  })
